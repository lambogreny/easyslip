...
// VerifySlip verifies a bank slip image.
//
// Example:
//
//	file, _ := os.Open("slip.jpg")
//	defer file.Close()
//	resp, err := client.VerifySlip(ctx, file, "slip.jpg")
func (c *Client) VerifySlip(ctx context.Context, image io.Reader, filename string) (*VerificationResponse, error) {
	pr, pw := io.Pipe()
	...
	// Create request
	req, err := c.newRequest(ctx, http.MethodPost, fmt.Sprintf("%s/verify", c.baseURL), pr)
	if err != nil {
		return nil, err
	}
